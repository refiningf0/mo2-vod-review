...
- **Every frame is read twice, through two image treatments.** Pushing contrast rescues a line in a bright scene and destroys one that was already legible -- a hit whose line was crisp to the eye came back as `You hit Moocifer for`, while the untreated crop read it whole. Neither treatment wins everywhere, so both readings go to the consensus. On a fight checked against the in-game log this went from three of five hits to five of five, and the clip that was already right stayed right. It costs about 60% more runtime, which is what the accuracy is worth.
- **Which treatment a reading came from travels with it.** Two readings of one line would otherwise look exactly like one line shown twice, and the dedupe counts copies in a frame to find overlapping lines -- every hit would come out doubled.
