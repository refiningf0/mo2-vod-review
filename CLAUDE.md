...
align_clocks      wall-clock lines and frame-timed lines onto one clock, per
                  recording -- a merged clip holds several
...
- **The clock offset is constant within a recording, not within a file.** A merged clip holds one offset per part, and the gap between them is however long the player went without recording: on one 4v8 the parts were 33 minutes apart and crossed midnight. Taking one median across the file picked the longest part and flung every event from the others thousands of seconds outside the clip, where the range check deleted them -- that report lost its first two minutes, eight hits visible in a single frame, and three players. `align_clocks` groups the estimates and shifts each line by its own group; a group needs 10% of the lines to count, since within one recording stray estimates sit up to a minute off and must still be absorbed.
...
- **Creatures do not "hit" you.** Each has its own verb -- `Terror Bird bites you for 5[Torso]`, `Ak Yaban Flayer slashes you`, `mauls`, `slices` -- and the pattern knew only "hits", so every mob attack in a clip was discarded: 139 readings of `bites` across nine fights, and a terror-bird fight missing 72 of its incoming damage. `MOB_VERBS` is a list, not "any word ending in s", because `X heals you for 24` is the same shape and is not damage. The log also grades the attack with an adverb (`deeply bites`), which the name swallowed until `ADVERB` took it.
- **An unknown flag is not a misreading of a shorter one.** `[Overhead]` contains `Head` whole and scored 0.67 against a 0.66 bar, so an overhead swing was filed as a hit to the head. A misreading loses or swaps letters (`Pany` for `Parry`); it does not keep the whole word and add three more. Flags seen in real reports that the vocabulary lacked -- `Underhew`, `Sling Thrust`, `Piercing Shot`, `Resist`, `Mind` -- are now in it, and `viewer.html` has its own `ABILITY` list that must learn the weapon ones too or the report will not count them.
